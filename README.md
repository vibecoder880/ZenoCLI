...
ZenoCLI ships as native release tarballs plus a GitHub-hosted npm tarball. Pick
...
# Windows (PowerShell)
powershell -c "irm https://github.com/vibecoder880/ZenoCLI/raw/khanh/scripts/install.ps1 | iex"

# …or install the latest release tarball directly (any OS with Node 22+)
npm install -g https://github.com/vibecoder880/ZenoCLI/releases/latest/download/zeno-cli.tgz
```

The shell/PowerShell installers download the matching per-OS asset
(`linux-x64`, `macos-x64`, `windows-x64`) from the latest GitHub release and
put `zeno` on your PATH. They respect `ZENO_VERSION`, `ZENO_PREFIX`, and
`ZENO_REPO` to pin a version, choose a prefix, or install from a fork.
